...
@Component({
  selector: 'app-king-all-jodi-data',
  standalone: true,
  imports: [RouterLink, MatButtonModule,MatInputModule,MatFormField, MatIcon, MatInputModule, FormsModule , NgxSearchFilterModule],
  templateUrl: './king-all-jodi-data.component.html',
  styleUrl: './king-all-jodi-data.component.scss'
})
export class KingAllJodiDataComponent implements OnInit {
  starlineData:any
  token:any
  term:any = ''
  animal: string = 'none';
  ...
  getStarlineData(){
    this.global.getWithToken("king-jodi",this.token).subscribe({
      next:(res:any)=>{
